...
const PurchasePR = () => {
    ...
    const renderTableHeader = () => {
        const tableHead = ['', 'S.No', 'Line No', 'Material No', 'Material Name', 'Material Unit', 'Price', 'Quantity', 'Total Price', 'Text', 'Delete', 'Created By', 'Date', 'Po No','Cost Center','Internal Order'];
        return (
            <tr>
                            ...
                            <tbody >
                                {
                                    data?.map((item, index) => {
                                        return <tr key={index}>
                                            ...
                                            <td>
                                                {view ? <DumyInput indum={item.material_text} /> : <>{item.po_no !== null ? <DumyInput indum={item.material_text} /> : <input type="text" value={item.material_text !== null ? item.material_text : ''} onChange={(e) => handleChange(e.target.value, 'material_text', index)} className="input input-bordered input-sm max-w-xs w-24" />}</>}
                                            </td>
                                            ...
                                            <td>
                                                {view ? <DumyInput indum={item.cost_center} /> : <>{item.po_no !== null ? <DumyInput indum={item.cost_center} /> : <input type="text" value={item.cost_center !== null ? item.cost_center : ''} onChange={(e) => handleChange(e.target.value, 'cost_center', index)} className="input input-bordered input-sm max-w-xs w-24" />}</>}
                                            </td>
